...
\titolo{Analisi, progettazione e implementazione di Metarace: un gioco 3D nel Metaverso}
\autore{Emanuele Pietropaolo}
\matricola{513489}
...
\usepackage{acronym} %per generare lista acronimi
\usepackage{csquotes}   % per la struttura delle quotes
\usepackage{listings}   %per l'aggiunta di snippet di codice
\usepackage{xcolor}
...
\capitolo{Background tecnologico}{capitolo-1}
\capitolo{Il core di Metarace}{capitolo-2}
\capitolo{Fase di Progettazione}{capitolo-3}
\capitolo{Fase di Implementazione}{capitolo-4}
%\capitolo{Risultati}{capitolo-5}
